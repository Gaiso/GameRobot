...
/**
 * Created by WangDongJie on 2016/11/1.
 */
...
public class Porter implements View.OnTouchListener {
    ...
    private ArrayMap<View, WindowManager.LayoutParams> mViewData = new ArrayMap<>();
    ...
    private void addAllView2Handler() {
        ...
        while (i.hasNext()) {
            View key = i.next();
            WindowManager.LayoutParams params = mViewData.get(key);
            addView2Handler(key, params);
        }
    }

    private void addView2Handler(View key, WindowManager.LayoutParams params) {
        int side = params.x >= (mDirectionHandler.getScreenWidth() / 2) ? DirectionHandler.RIGHT_SIZE :
    ...
    public void addTarget(View target, WindowManager.LayoutParams params) {
        if (!mViewData.containsKey(target)) {
            mViewData.put(target, params);
            mManager.addView(target, params);
            if (mIsAttach) {
    ...
    public void removeTarget(View target) {
        if (mViewData.containsKey(target)) {
            mViewData.remove(target);
            mManager.removeView(target);
            mDirectionHandler.removeView(target);
        }
    }

    public boolean hasTarget(View target) {
        return mViewData.containsKey(target);
    }

    private void move(View target, float x, float y) {
        WindowManager.LayoutParams params = mViewData.get(target);
        if (params == null)
            return;
        params.x = (int) x;
        params.y = (int) y;
        mManager.updateViewLayout(target, params);
    }
